...
// ---- Optional controller lifecycle hooks ----

// BeforeActioner can be implemented by a controller to run logic before its
// action method. Returning false aborts the request — BeforeAction is
// responsible for writing a response in that case (e.g. 401 Unauthorized).
type BeforeActioner interface {
	BeforeAction(w http.ResponseWriter, r *http.Request) bool
}

// AfterActioner can be implemented by a controller to run logic after its
// action method has completed.
type AfterActioner interface {
	AfterAction(w http.ResponseWriter, r *http.Request)
}
...
// Modifier that also runs BeforeAction/AfterAction if the resolved controller
// implements them.
//
...
// BeforeAction instead, using the request/context.
func buildControllerAction(container *Container, controller interface{}, methodName string) Modifier {
	...
	before, hasBefore := resolved.(BeforeActioner)
	after, hasAfter := resolved.(AfterActioner)

	return func(w http.ResponseWriter, r *http.Request) {
		if hasBefore && !before.BeforeAction(w, r) {
			return // BeforeAction aborted the request and already wrote a response
		}
		...
		if hasAfter {
			after.AfterAction(w, r)
		}
	}
}

// resolveController checks whether the controller defines a Loader(...) method.
// If it does, each parameter type is resolved from the container (matched as
// an interface type) and Loader is called; whatever it returns becomes the
// "real" controller instance used for the action lookup and before/after hooks.
// If there's no Loader method, the controller is used as-is.
func resolveController(container *Container, controller interface{}) interface{} {
	value := reflect.ValueOf(controller)
	loader := value.MethodByName("New")
	if !loader.IsValid() {
		return controller
	}

	if container == nil {
		panic(fmt.Sprintf("customrouter: %T defines Loader(...) but no Container was configured — call router.UseContainer(...)", controller))
	}
	...
	if len(results) == 0 {
		panic(fmt.Sprintf("customrouter: %T.Loader must return the initialized controller", controller))
	}
